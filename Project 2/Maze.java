 ...
 * 
 * ASSIGNMENT 2-Part2
 * STUDENT AUTHOR:  <Sezen Zeynep Sumer>
 *
 * MODIFY 
 * 
 * */
...
public class Maze {
  ...
  public boolean solveMaze(iSimpleContainer<Coordinate> sc, iDeque<Coordinate> visitedNodes, String mazeName)
	...
	while (!sc.isEmpty()) {
    	...
    	Coordinate neighborD = new Coordinate(currentState.x, currentState.y-1);
    	Coordinate neighborR = new Coordinate(currentState.x+1, currentState.y);
    	Coordinate neighborU = new Coordinate(currentState.x, currentState.y+1);
    	Coordinate neighborL= new Coordinate(currentState.x-1, currentState.y);
    	Coordinate [] neighbors = {neighborD, neighborR, neighborU, neighborL};
    	for(Coordinate neighbor : neighbors) {
    		if (isInMaze(neighbor)) sc.push(neighbor);	
    	}
	}
	}
    	return false;
  ...
  private Coordinate getStartState(char[][] maze) {
	  for(int i=0; i<rows; i++) {
		  for(int j = 0; j<cols; j++) {
			 char temp =  maze[i][j];
			 if (temp == 'S') return new Coordinate(i,j);	 
		  }
	  }
    return null;
  }
